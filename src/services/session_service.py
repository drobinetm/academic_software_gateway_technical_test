"""Async service to persist and remove session records."""
...
class SessionService:
    ...
    async def save_session(self, record: SessionRecord) -> None:
        """Upsert a session keyed by its token (idempotent)."""
    ...
    async def delete_session_by_token(self, token: str) -> bool:
        """Delete the session with the given token. Returns True if removed."""
